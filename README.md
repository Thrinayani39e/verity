...
3. **Memory should see patterns no single claim reveals.** A fraud ring rarely looks suspicious one claim at a time — three claims filed under three different names, weeks apart, are each individually unremarkable. What ties them together (a shared bank account, a shared address) is only visible by querying *across* every claim ever filed. `src/verity/fraud_ring.py` does exactly that against CockroachDB's relational data, surfaced live at `/fraud-rings` and in the "Fraud Rings" dashboard page. A stateless, one-claim-at-a-time agent cannot see this by construction — there's nothing wrong with its reasoning, it simply never has the cross-claim view.
...
**Fraud Ring Buster** (`/fraud-rings`, `src/verity/fraud_ring.py`) is a third: claims that share a bank account or address across different claimant names look completely unremarkable evaluated one at a time — nothing in any single claim would trip a review threshold. The pattern only exists in the relationship *between* claims, reconstructed with a real SQL query (`GROUP BY ... HAVING count(*) > 1`) against the same store every other claim lives in, and rendered as a force-directed graph on the dashboard so the ring is something a reviewer can see and click into, not just a number in a table.
