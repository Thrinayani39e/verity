...
## Why this isn't "another agentic insurance claims demo"

"Multi-agent, insurance claims, fraud detection, on AWS" is not a novel combination on its own. AWS publishes its own reference architecture for exactly this pairing: [`aws-samples/sample-agentic-insurance-claims-processing-eks`](https://github.com/aws-samples/sample-agentic-insurance-claims-processing-eks), a production-style LangGraph app with four persona portals (claimant, adjuster, SIU, supervisor) and ML-scored fraud risk, deployed on EKS. It's a legitimate, well-built reference, and it's worth naming directly rather than hoping nobody notices the overlap.

The overlap is real at the domain level and stops there. Their stack is MongoDB on Kubernetes: a document store with no serializable-by-default isolation, no equivalent of `AS OF SYSTEM TIME`, and fraud detection implemented as a separately-trained ML risk score attached to each claim individually, not a query that traces relationships *across* claims. Nothing in that architecture claims a concurrency-safety guarantee for concurrent agents, a database-enforced uniqueness constraint on money movement, or point-in-time replay of what an agent knew.

Those are exactly the four things this project is actually built around, and none of them are portable to that stack without the application rebuilding, in code, guarantees CockroachDB provides structurally: `claim_next_pending()`'s proven zero-double-claim guarantee under real concurrent load, `payouts.claim_id UNIQUE` making a double payout a schema-level impossibility rather than an application bug waiting to happen, `AS OF SYSTEM TIME` reconstructing exact historical state rather than a cached snapshot, and `fraud_ring.py`'s connected-components graph traversal finding rings *across* claims rather than scoring one claim at a time. The insurance domain is the vehicle for demonstrating these; it was never meant to be the pitch by itself.
